import { Text, View, StyleSheet, Dimensions, Pressable, Modal, TextInput } from 'react-native';
...
export default function Mine() {
 ...
 const {isLogin,setIsLogin,user}=useGroups()
  ...
  const editname=async()=>{
    try{
      ...
      const response3=await post("http://8.129.3.142:8080/user/update",userData,true)
      Alert.alert("修改用户名成功")
  ...
  const pickImageAsync = async () => {
    ...
    if (!result.canceled) {
      ...
      try{
        ...
        formData.append('file', { uri:imgUrl,name:'upload.jpg', type: 'image/jpeg' } as any) ;
        ...
        const response=await put("http://8.129.3.142:8080/image/user/update",userData,true)
  ...
  useEffect(() => {
    const getUserData = async () => {
       try{
         const response1=await get(`http://8.129.3.142:8080/user/info/${id}`,true)
       ...
       try{
           const response2=await get(`http://8.129.3.142:8080/image/user/get/${id}`,true)
    ...
    >
      <View style={styles.headerregion}>
        ...
        >
          <Image
            ...
            style={{ width: deviceWidthDp * 0.2, height: deviceWidthDp * 0.2,borderRadius:50 }}
          />
        </Pressable>
        ...
        <Pressable
          ...
          style={{ position: 'absolute', left: deviceWidthDp * 0.35, top: deviceHeightDp * 0.14 }}
        >
          <Text style={{ fontSize: 20, letterSpacing: 2, color: '#000000', fontWeight: '500' }}>
            {username}
          </Text>
        </Pressable>
        <Pressable
          style={{ position: 'absolute', left: deviceWidthDp * 0.85, top: deviceHeightDp * 0.15 }}
        >
          <Image
            source={require('../../assets/images/退出键.png')}
            style={{ width: deviceWidthDp * 0.04, height: deviceWidthDp * 0.04 }}
          />
        </Pressable>
      </View>
      ...
      <View style={styles.mediumregion}>
        <View style={{ flex: 1 / 3, alignItems: 'center', padding: 1 }}>
          ...
          <Text style={{ position: 'absolute', left: deviceWidthDp * 0.12, top: deviceHeightDp * 0.01 }}>
            {number1}
          </Text>
        </View>
        <View style={{ flex: 1 / 3, alignItems: 'center', padding: 1 }}>
      ...
      <View style={styles.bottomregion}>
        <Pressable
          ...
          style={{ flex: 1 / 4, backgroundColor: 'transparent', borderBottomWidth: 0.2, borderColor: 'grey' }}
        >
          {isLogin ? (
            <View>
              <Image
