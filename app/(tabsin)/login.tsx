...
export default function LoginPage() {
  ...
  const { setIsLogin, setUser, email, setEmail,token,setToken } = useGroups();
    ...
    if (isEmailEmpty || isPasswordEmpty) {
      ...
      try {
        ...
        const response = await post(
          "http://8.129.3.142:8080/user/login",
          loginData
        );
        if (response.ok) {
      ...
      <View style={styles.form}>
        ...
        <View style={styles.functionregion}>
          ...
          >
            ...
            <Link href="/(tabsin)/register" style={{ color: "#A5A5A5" }}>
              注册新用户
            </Link>
          </Pressable>
        </View>
