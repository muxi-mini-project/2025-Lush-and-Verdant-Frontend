...
export default function ForgetPassword(){
    ...
    const handleForgetPassword = async() => {
        ...
        try{
            ...
            const response=await post("http://8.129.3.142:8080/user/foralt",forgetPasswordData)
            if(response.ok){
                const result=await response.json()
                alert("您已经重新注册成功")
    ...
    return(
        ...
        <View>
         ...
         <TextInput style={styles.input1}  value={email} onChangeText={setEmail}  placeholder='请输入邮箱' keyboardType='email-address'></TextInput><Pressable style={{position:'absolute', left:deviceWidthDp*0.2,top:deviceHeightDp*0.2,width:75}}><Text style={{color:'#A0D4A3'}}>获取验证码</Text></Pressable>
